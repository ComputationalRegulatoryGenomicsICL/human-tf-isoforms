...
```{r, include=T}
ips_domains_ipr_ens99 = read.delim(file = protein_fasta_tfs_name, 
                                   header = F, 
                                   row.names = NULL, 
...
general_dbd_overlap_table = general_dbd_overlap_table %>%
  # read.delim("analysis/general_dbd_overlap_table.tsv") %>%
  mutate(start = as.character(start)) %>%
  mutate(stop = as.character(stop)) %>%
...
The `ensg_enst_tsl_99.tsv` table was obrained with the following code:
...
# Domain and DBD- isoform analysis

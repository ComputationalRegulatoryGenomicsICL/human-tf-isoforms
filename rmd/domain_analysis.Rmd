...
```{r, include=T}
ensg_enst_ensp = read.delim(ensembl99_all_name)
```
...
write.table(ensg_enst_ensp, file = "ensg_enst_ensp_99.tsv", quote = F, sep = "\t", row.names = F)
```
...
write.table(gene_biotype_table, file = "tables/gene_biotype_table_99.tsv", quote = F, sep = "\t", row.names = F)
#gene_biotype_table = read.delim("gene_biotype_table_99.tsv")
...
The table was obtained using the following code:

```{r, include=T}
# write.table(ensg_enst_ensp %>% filter(ensembl_gene_id %in% tfs$Ensembl.ID), file = "ensg_enst_ensp_99_tfs.tsv", quote = F, sep = "\t", row.names = F)
```
...
```{r, include=T}
interpro_entries = read.delim("data/interpro77.0/entry.list")
```
...
```{r, include=T}
# gene_biotype_table = getBM(attributes = "gene_biotype", filters = "ensembl_gene_id", values = c("ENSG00000249459", "ENSG00000214189", "ENSG00000228623"), mart = ensembl)
# write.table(gene_biotype_table, file = "data/ensembl99/gene_biotype_table_99.tsv", quote = F, sep = "\t", row.names = F)
gene_biotype_table = read.delim("data/ensembl99/gene_biotype_table_99.tsv")
...
```{r, include=T}
# tx_ipr_sign = ips_domains_ipr_ens99 %>% 
#   mutate(evalue = as.numeric(as.character(evalue))) %>%
#   filter(evalue < 1) %>%
#   mutate(tx_ipr = paste0(as.character(ensembl_transcript_id),
#                          "_",
#                          as.character(ipr_accession))) %>%
#   pull(tx_ipr) %>%
#   unique()
# 
# tx_ipr_nonsign = ips_domains_ipr_ens99 %>% 
#   mutate(evalue = as.numeric(as.character(evalue))) %>%
#   filter(evalue >= 1) %>%
#   mutate(tx_ipr = paste0(as.character(ensembl_transcript_id),
#                          "_",
...
#   unique()
# 
# tx_ipr_na = ips_domains_ipr_ens99 %>% 
#   mutate(evalue = as.numeric(as.character(evalue))) %>%
#   filter(is.na(evalue)) %>%
#   mutate(tx_ipr = paste0(as.character(ensembl_transcript_id),
#                          "_",
...
#   unique()
# 
# length(unique(c(tx_ipr_sign, tx_ipr_nonsign, tx_ipr_na)))
# 
# nonsign_na_only = setdiff(tx_ipr_nonsign, tx_ipr_sign)
# nonsign_only = setdiff(nonsign_na_only, tx_ipr_na)
# na_only = setdiff(nonsign_na_only, tx_ipr_nonsign)
# 
# # ENST00000276127_IPR013087
# ips_domains_ipr_ens99 %>%
#   mutate(tx_ipr = paste0(as.character(ensembl_transcript_id),
#                          "_",
#                          as.character(ipr_accession))) %>%
#   filter(tx_ipr %in% nonsign_na_only) %>%
#   dplyr::select(ensembl_gene_id, ensembl_transcript_id, ipr_accession, ipr_description, db, domain_accession, evalue, start, stop, status) %>%
#   arrange(ensembl_gene_id, ensembl_transcript_id, as.numeric(start), as.numeric(stop)) #%>%
# #
#   # dplyr::select(ensembl_transcript_id, ipr_accession, tx_ipr) %>%
#   # distinct() #%>%
#   # nrow()
# 
# ips_domains_ipr_ens99 %>%
...
#   nrow()
# 
# nrow(ips_domains_ipr_ens99_groupped)
# 
#   group_by(ensembl_transcript_id, ipr_accession) %>%
#   mutate()nrow() %>%
#   ungroup()
#   dplyr::select(ensembl_gene_id, ensembl_transcript_id, ipr_accession, ipr_description, db, evalue, start, stop) %>%
#   arrange(ensembl_gene_id, ensembl_transcript_id, as.numeric(start), as.numeric(stop))
```

...
domain_classification_table_name = args[14]
...
writeLines(c(ensg_enst_ensp_name, tfs_name, ips_domains_ipr_ens99_final_strat_name), fileConn)
close(fileConn)

ensg_enst_ensp = read.delim(ensg_enst_ensp_name)

tfs = read.delim(tfs_name)

ips_domains_ipr_ens99_final_strat = read.delim(ips_domains_ipr_ens99_final_strat_name)
```
...
protein_fasta = read.fasta(file = protein_fasta_tfs_99_name, seqtype = "AA")
...
```{r, include=T}
ips_domains_ipr_ens99_int_specific_retained = read.delim(ips_domains_ipr_ens99_int_specific_retained_name)

domain_classification_table = read.delim(domain_classification_table_name)
...
```{r, include=T}
ensg_enst_tsl = read.delim(ensg_enst_tsl_name,
                           stringsAsFactors = F) %>%
  dplyr::select(ensembl_gene_id,
                ensembl_transcript_id,
                transcript_tsl,
